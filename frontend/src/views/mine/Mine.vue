    ...
    <div class="mine_box">
      ...
      <div class="detail">
        <div class="mine_nav">
          ...
          <!-- <div class="nav_" @click="selectNavItem('观看历史')">
            <span class="nav_span">观看历史</span>
          </div> -->
        </div>

        <!-- 根据选中的导航项来显示内容 -->
        <div class="mine_blank" v-if="selectedNavItem === '作品'">
          <template>
            <div class="discover">
                  ...
                  >
                    <div class="grid-content bg-purple">
                      <div
                        class="img_box"
                        @mouseover="
                          videoPlayback(index, videoArr3[index].video_url)
                        "
                        @mouseout="
                          videoStopped(index, videoArr3[index].video_url)
                        "
                        @click="toPlayer(item)"
                      >
                        <img :src="videoArr3[index].thumb" alt="" ref="img" />
                      ...
                      <!-- 标题 -->
                      <div class="tit">
                        <span>{{ videoArr3[index].title }}</span>
                      </div>
                      <!-- 用户及视频上传时间 -->
                      <div class="actcont-auto">
                        <img :src="videoArr3[index].avatar" alt="" />
                        <span class="uname">{{
                          videoArr3[index].username
                        }}</span>
                        <span class="mtime">{{ videoArr3[index].mtime }}</span>
                      </div>
                    </div>
                  </el-col>
                </el-row>
        ...
        <div class="mine_blank" v-if="selectedNavItem === '收藏'">
          <template>
            <div class="discover">
              <div class="video_content">
                <el-row :gutter="16">
                  ...
                  >
                    <div class="grid-content bg-purple">
                      <div
                        class="img_box"
                        @mouseover="
                          videoPlayback(index, videoArr2[index].video_url)
                        "
                        @mouseout="
                          videoStopped(index, videoArr2[index].video_url)
                        "
                        @click="toPlayer(item)"
                      >
                        <img :src="videoArr2[index].thumb" alt="" ref="img" />
                        <!-- contextmenu(禁止右键) muted(解决谷歌浏览器不能自动播放)-->
                        <div class="video_box">
                          <video
                            id="video"
                      ...
                      <!-- 标题 -->
                      <div class="tit">
                        <span>{{ videoArr2[index].title }}</span>
                      </div>
                      <!-- 用户及视频上传时间 -->
                      <div class="actcont-auto">
                        <img :src="videoArr2[index].avatar" alt="" />
                        <span class="uname">{{
                          videoArr2[index].username
                        }}</span>
                        <span class="mtime">{{ videoArr2[index].mtime }}</span>
                      </div>
                    </div>
                  </el-col>
                </el-row>
...
export default {
  ...
  mounted() {
    this.isLoggedIn(), this.videoList(), this.videoList2(), this.videoList3();
    setInterval(() => {
      this.isLoggedIn(), this.videoList(), this.videoList2(), this.videoList3();
    }, 1000);
    // this.avatar()
  ...
  methods: {
    setUserMsg() {
      ...
      Login(data)
        .then((response) => {
          ...
          this.follow_count = msg.follow_count;
          this.follower_count = msg.follower_count;
          this.total_favorited = msg.total_favorited;
          //   console.log(this.follow_count);
        })
        .catch((error) => {
          // 处理登录失败的情况
          //  console.log('获取信息失败');
      ...
      //   const userMsg = JSON.parse(localStorage.getItem("userMsg"));
      //   console.log(userMsg);
      //   this.follow_count = userMsg.follow_count;
      //   this.follower_count = userMsg.follower_count
      //   this.total_favorited = userMsg.total_favorited
    },
    videoList() {
      let data1 = {};
      const userMsg = JSON.parse(localStorage.getItem("userMsg"));
    ...
    videoList2() {
      ...
      if (isLogin) {
        ...
        this.$http.like.getCollectVideoByUser(data1).then(({ data }) => {
          //   console.log(data);
          this.videoArr2 = data;
          // this.videoArr = data.list;
          //   console.log(this.videoArr2);
        });
      }
    },
    videoList3() {
      ...
      if (isLogin) {
        ...
        this.$http.video.findUploadVideoById(data1).then(({ data }) => {
            // console.log(data);
          this.videoArr3 = data;
          // this.videoArr = data.list;
          //   console.log(this.videoArr2);
