...
type Relation struct {
	...
	FromUserId int       `gorm:"index:idx_from_user;index:idx_relationship,unique" json:"from_user_id"`
	ToUserId   int       `gorm:"index:idx_to_user;index:idx_relationship,unique" json:"to_user_id"`
...
func (relation *Relation) AfterCreate(tx *gorm.DB) (err error) {
	// 1. 被关注者的粉丝数+1
	err = tx.Model(&User{}).Where("id = ?", relation.ToUserId).
	...
	// 2. 关注者的关注数+1
	err = tx.Model(&User{}).Where("id = ?", relation.FromUserId).
